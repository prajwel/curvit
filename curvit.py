...
framecount_per_sec = 28.7  # 1 second = ~29 frames for 512x512 mode.
...
sky_radius = 12 # radius of sky aperture in pixels.
...
def makecurves(events_file = events_file,
    ...
    np.savetxt(events_file +"_detected.coo", uA, fmt = '%4.f\t%4.f')
    ...
    def create_sub_image(pos_x, pos_y, sub_size, cir_rad, sub_name):
        ...
        plt.gcf().gca().add_artist(obj_circle)
        source_png_name = events_file + sub_name + ".png"
        plt.savefig(source_png_name, format = 'png', bbox_inches = 'tight')
        plt.clf()    
        
    create_sub_image(x_bg, y_bg, sub_fig_size, sky_radius, '_background')
    ...
    for uaxy in uA:
        ...
        figname = str(xp) + '_' + str(yp) + '_' + events_file + '_' + str(median_time) + "_light_curve.png"
